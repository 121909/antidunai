...
解析输出不依赖 Telegram `bot` 标志。关联优先级依次为回复消息 ID、视频说明文字或 `Source` 隐藏链接等位置提取出的原链接、配置解析发送者的下一条视频 FIFO。链接源消息即使被发送者立即删除，已经登记的内存关联仍保留到 `BURST_GUARD_IDEMPOTENCY_TTL_SECONDS`。只有实际含视频媒体且能关联到候选的解析消息才可能删除；普通消息仍被忽略。同一原链接同时属于保留项和淘汰项时不会按链接删除，以避免误伤保留视频。
