...
`burst-guard` 是一个独立运行的 Telegram userbot。每个群组按非机器人消息顺序切分为固定的 `X` 条消息一组；一组内指定用户的视频链接消息达到 `Y` 条时，服务等概率随机保留 1 条并删除其余链接消息。它不下载媒体、不读取链接页面，也不扫描历史消息。

首个版本只支持单实例运行，并且必须使用专门注册的 Telegram 用户账号。不要使用个人日常账号；userbot 的使用还必须符合 Telegram 平台规则和适用法律。

## 行为

- 只有指定用户发送且包含配置视频域名 URL 的消息才是候选；直接发送的 `video`、`video_note` 或视频文档不计入 `Y`。
...
- `BURST_GUARD_THRESHOLD` 是触发随机保留的链接消息数 `Y`。
- `X`、`Y` 都必须为正整数，且 `X >= Y`。
