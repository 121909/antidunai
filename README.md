...
- 机器人和 Telegram 服务消息不会触发窗口更新。解析服务无论使用 bot、userbot 还是频道身份，只要其视频携带被淘汰候选的原链接，就会与源消息一同删除或在延迟到达时立即删除。
...
解析输出关联不依赖回复关系或 Telegram `bot` 标志：服务会从视频说明文字、`Source` 等隐藏 `text_link`、URL 按钮和网页预览中提取原链接，并在内存中保留到 `BURST_GUARD_IDEMPOTENCY_TTL_SECONDS`。只有实际含视频媒体、链接域名已配置且原链接与被淘汰候选匹配的非目标消息才会删除；普通消息仍被忽略。同一原链接同时属于保留项和淘汰项时不会按链接删除，以避免误伤保留视频。
